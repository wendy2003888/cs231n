...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  h = np.zeros((N, T + 1, H))
  cache = range(T + 1)
  h[:,0,:], cache[0] = h0, (x[:,0,:], h0, Wx, Wh, b)
  for i in xrange(T):
    h[:,i + 1,:], cache[i + 1] = rnn_step_forward(x[:,i,:], h[:,i,:], Wx, Wh, b)
  h , cache = h[:,1:,:], cache[1:]
...
def rnn_backward(dh, cache):
  ...
  for i in xrange(T - 1, -1, -1):
    dx_, dprev_h, dWxi, dWhi, dbi = rnn_step_backward(dh[:,i,:] + dprev_h, cache.pop())
    dx[:,i,:] = dx_
    dWx += dWxi
    dWh += dWhi
    db += dbi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = (x, prev_h, prev_c, Wx, Wh, a, i, f, o, g, next_h, next_c)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  x, prev_h, prev_c, Wx, Wh, a, i, f, o, g, next_h, next_c = cache
  N, H = dnext_h.shape
  ai, af, ao, ag = np.split(a, 4, axis=-1)
  # ai, af, ao, ag = a[:, :H], a[:, H:2*H], a[:, 2*H:3*H], a[:, 3*H:]
  ...
  di = dnext_c * g
  dg = dnext_c * i
  dai, daf, dao = di * i * (1 - i), df * f * (1 - f), do * o * (1 - o)
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  h[:,0,:] = h0
  for i in xrange(T):
    h[:,i+1,:], c[:,i+1,:], cache[i+1] = lstm_step_forward(x[:,i,:], h[:,i,:], c[:,i,:], Wx, Wh, b)
  h, cache = h[:,1:,:], cache[1:]
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_backward(dh, cache):
  ...
  for i in xrange(T - 1, -1, -1):
    dx[:,i,:], dprev_h, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(dh[:,i,:] + dprev_h, dnext_c, cache[i])
    dnext_c = dprev_c
    dWx += dWxi
    dWh += dWhi
    db += dbi
